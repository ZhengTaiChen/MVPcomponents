...
/*
    public void onDestroy() {
        if (PmApp.mLong == null)
            PmApp.mLong = new HashMap<>();
        PmApp.mLong.put(TIME, time);
        PmApp.mLong.put(CTIME, System.currentTimeMillis());
        clearTimer();
...
*/
    ...
    public void onCreate(Bundle bundle) {
       /* Log.e("yung", PmApp.mLong + "");
        if (PmApp.mLong == null)
            return;
        if (PmApp.mLong.size() <= 0)// 这里表示没有上次未完成的计时
            return;
        long time = System.currentTimeMillis() - PmApp.mLong.get(CTIME) - PmApp.mLong.get(TIME);
        PmApp.mLong.clear();*/
        if (time > 0)
            return;
        else {
